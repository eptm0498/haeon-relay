...
await test('비서 API 동작과 장애 처리', async t => {
  ...
  try {
    await t.test('상태 확인과 단순 변경은 외부 요청 0회', async () => {
      ...
      assert.equal((await (await GET(new Request('http://localhost/api/biseo'))).json()).workUsageDetection, false);
    ...
    await t.test('강제 사용은 단순 작업도 실제 생성, 성공 토큰 반환', async () => {
      let calls = 0;
    ...
    await t.test('Astra 장애는 Claude 1회 대체, 성공 결과 보존', async () => {
      const models = [];
      ...
      const data = await (await POST(request({ task: '프로그램 오류 분석' }))).json();
    ...
    await t.test('빈 응답도 대체 모델로 복구', async () => {
      ...
      assert.equal((await (await POST(request({ task: '설계' }))).json()).ok, true); assert.equal(calls, 2);
    });
    ...
    await t.test('양쪽 장애는 명시적 실패', async () => {
      globalThis.fetch = async () => new Response('unavailable', { status: 503 });
      const response = await POST(request({ task: '설계' })); const data = await response.json(); assert.equal(response.status, 502); assert.equal(data.ok, false); assert.equal(data.meta.calls, 2);
    });
    await t.test('취소한 요청은 새 외부 호출 없이 종료', async () => {
      globalThis.fetch = async () => { assert.fail('unexpected call'); };
      const controller = new AbortController(); controller.abort();
      const response = await POST(new Request('http://localhost/api/biseo', { method: 'POST', body: JSON.stringify({ task: '설계' }), signal: controller.signal })); assert.equal(response.status, 504);
    });
